...
This file records the Slurm workflow and successful jobs. JSON latency fields
are seconds and energy fields are joules; console summaries usually print
...
| `slurm/update_tall_matmul_figures.sbatch` | Regenerates tall figures from an existing `RUN_ROOT`. | Tall figure directory. |
...
| GPT-3 175B | 10 decomposed single-einsum workloads, repeated across 96 layers | Ring, mesh, torus, circulant | `logs/slurm-gpt3-175b-12632099/results.json` |
...
| unknown | unknown | unknown | MoE expert-FFN AccelForge run used for `figures/current_moe_expert_ffn/`. | Successful; result summary and paper figure are available. | `figures/current_moe_expert_ffn/moe_expert_ffn_summary.csv` |
...
| --- | --- | --- |
| `12524857` | Seven-workload small matmul run. | `logs/slurm-12524857/results.json` |
| `12634419` | Exact rerun of the seven unbatched `12524857` workloads. | `logs/slurm-12634419/results.json` |
| `12634425` | Batched versions of the seven `12524857` workloads at `B=256,512,1024`. | `logs/slurm-batched-matmuls-12634415/results.json` |
...
- The cost model computes each collective independently. It does not
  merge link loads across different collectives.
- The paper matmul runs use `MAP_CHIPS=64` and `EVAL_CHIPS=64`.
