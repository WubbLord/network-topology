...
Maps on MAP_CHIPS chips, scales data movement to EVAL_CHIPS chips, and evaluates
candidate topologies. Defaults are MAP_CHIPS=8 and EVAL_CHIPS=64.
...
def _default_accelforge_root() -> Path:
    env_root = os.environ.get("ACCELFORGE_ROOT")
    if env_root:
        return Path(env_root).expanduser()

    checkout_root = SCRIPT_DIR.parent / "accelforge"
    ...
    try:
        ...
        return checkout_root
    return Path(accelforge.__file__).resolve().parent.parent


ACCELFORGE = _default_accelforge_root()
ARCH = SCRIPT_DIR / "accelforge_configs" / "tpu_v4_distributed_1d.yaml"
MAP_CHIPS = int(os.environ.get("MAP_CHIPS", 8))
EVAL_CHIPS = int(os.environ.get("EVAL_CHIPS", 64))
if MAP_CHIPS <= 0 or EVAL_CHIPS <= 0:
    raise ValueError("MAP_CHIPS and EVAL_CHIPS must be positive integers.")
if EVAL_CHIPS % MAP_CHIPS != 0:
    raise ValueError("EVAL_CHIPS must be an integer multiple of MAP_CHIPS.")
...
DECOMPOSED_GPT3_MODEL = "gpt3_175b_decomposed_einsums"

GPT3_EINSUM_SPECS = [
    {
        "name": "I",
        "equation": "I[b, m, d] = I_in[b, m, d]",
        "renames": {"input": "I_in", "output": "I"},
        "is_copy_operation": True,
    },
    {
        "name": "V",
        "equation": "V[b, m, h, e] = I[b, m, d] * WV[h, e, d]",
        "renames": {"input": "I", "weight": "WV", "output": "V"},
    },
    {
        "name": "K",
        "equation": "K[b, m, h, e] = I[b, m, d] * WK[h, e, d]",
        "renames": {"input": "I", "weight": "WK", "output": "K"},
    },
    {
        "name": "Q",
        "equation": "Q[b, m, h, e] = I[b, m, d] * WQ[h, e, d]",
        "renames": {"input": "I", "weight": "WQ", "output": "Q"},
    },
    {
        "name": "QK",
        "equation": "QK[b, m, p, h] = Q[b, m, h, e] * K[b, M: p, h, e]",
        "renames": {"input": "Q", "weight": "K", "output": "QK"},
    },
    {
        "name": "QK_softmax",
        "equation": "QK_softmax[b, m, p, h] = QK[b, m, p, h]",
        "renames": {"input": "QK", "output": "QK_softmax"},
    },
    {
        "name": "AV",
        "equation": "AV[b, m, h, f] = QK_softmax[b, m, p, h] * V[b, M: p, h, E: f]",
        "renames": {"input": "QK_softmax", "weight": "V", "output": "AV"},
    },
    {
        "name": "Z",
        "equation": "Z[b, m, g] = AV[b, m, h, f] * WZ[h, f, g]",
        "renames": {"input": "AV", "weight": "WZ", "output": "Z"},
    },
    {
        "name": "FFA",
        "equation": "FFA[b, m, c] = Z[b, m, g] * WFFA[g, c]",
        "renames": {"input": "Z", "weight": "WFFA", "output": "FFA"},
    },
    {
        "name": "FFB",
        "equation": "FFB[b, m, j] = FFA[b, m, c] * WFFB[c, j]",
        "renames": {"input": "FFA", "weight": "WFFB", "output": "FFB"},
    },
]
...
MATMUL_12524857_WORKLOADS = [
    ("Small 2Kx8K", 2048, 8192),
    ("Small 4Kx16K", 4096, 16384),
    ("Medium 8Kx32K", 8192, 32768),
    ("Medium 16Kx32K", 16384, 32768),
    ("Wide 8Kx256K", 8192, 262144),
    ("VeryWide 2Kx256K", 2048, 262144),
...
def _is_decomposed_gpt3_workload(params) -> bool:
    return params.get("__decomposed_model") == DECOMPOSED_GPT3_MODEL


def _accelforge_jinja_params(params):
    return {key: value for key, value in params.items() if not key.startswith("__")}


def _decomposed_gpt3_config(params):
    ...
    return {
        ...
        "einsums_per_layer": len(GPT3_EINSUM_SPECS),
        "einsums": [spec["name"] for spec in GPT3_EINSUM_SPECS],
        "note": (
            "Each GPT-3 einsum is mapped independently with the same proxy feedback "
            "loop as the matmul workloads, then replicated across layers."
        ),
    }


def _yaml_inline_mapping(mapping):
    return "{" + ", ".join(f"{key}: {value}" for key, value in mapping.items()) + "}"


def _gpt3_einsum_workload_yaml(spec):
    lines = [
        "# Generated by sweep_matmuls.py for decomposed GPT-3 mapping.",
        "workload:",
        "  rank_sizes:",
        ...
        "",
        "  einsums:",
        f"  - einsum: \"{spec['equation']}\"",
    ]
    if spec.get("is_copy_operation"):
        lines.append("    is_copy_operation: True")
    lines.extend(
        [
            f"    renames: {_yaml_inline_mapping(spec['renames'])}",
            "",
            "renames:",
            "  einsums:",
            "  - name: default",
...
def _write_gpt3_einsum_workloads(workload_dir: Path):
    workload_dir.mkdir(parents=True, exist_ok=True)
    entries = []
    for index, spec in enumerate(GPT3_EINSUM_SPECS):
        workload_path = workload_dir / f"{index:02d}_{_safe_path_part(spec['name'])}.yaml"
        workload_path.write_text(_gpt3_einsum_workload_yaml(spec), encoding="utf-8")
        entry = dict(spec)
        entry["path"] = workload_path
        entries.append(entry)
    return entries


def _copy_transfer_with_einsum_prefix(transfer: NetworkTransfer, prefix: str):
    return NetworkTransfer(
        tensor_name=f"{prefix}:{transfer.tensor_name}",
        data_bytes=float(transfer.data_bytes),
        collective_type=transfer.collective_type,
        src_chip=transfer.src_chip,
        dst_chips=None if transfer.dst_chips is None else list(transfer.dst_chips),
        participating_chips=(
            None
            if transfer.participating_chips is None
            else list(transfer.participating_chips)
        ),
    )


def _copy_decision_with_einsum_prefix(decision, prefix: str):
    copied = dict(decision)
    copied["einsum"] = f"{prefix}:{decision['einsum']}"
    return copied
...
def _collective_signature(decisions):
    return sorted(
        (
            ...
            decision["collective_type"] or "LOCAL",
            decision["proxy_action"] or "",
...
def make_workloads(workloads_dir: Path):
    ...
    return [
        ...
        # === Batched variants of the completed slurm-12524857 workload set ===
        *[
            (
        ...
        *_square_sweep_workloads(square_matmul_template),
        ...
        ("Medium 8Kx32K", matmul_template,
         {"N_EINSUMS": 1, "M": 8192, "KN": 32768}),
        ("Medium 16Kx32K", matmul_template,
...
def map_and_extract(path, params, network_proxies, af, map_workload_to_arch):
    ...
    spec = af.Spec.from_yaml(
        ...
        jinja_parse_data={
            "NUM_CHIPS": MAP_CHIPS,
            **network_proxies,
            **_accelforge_jinja_params(params),
        },
    )
...
def _combine_decomposed_mapping_cost_summaries(layer_entries, layer_count, config):
    numeric_keys = [
        "proxy_total_energy_scaled",
        "proxy_non_network_energy_scaled",
        "proxy_network_energy_scaled",
        "proxy_total_latency",
        "proxy_total_latency_reconstructed",
        "non_network_total_latency_reconstructed",
    ]
    combined = {key: 0.0 for key in numeric_keys}
    energy_by_component = defaultdict(float)

    for entry in layer_entries:
        summary = entry["mapping_result"]["mapping_cost_summary"]
        for key in numeric_keys:
            combined[key] += float(summary.get(key, 0.0)) * layer_count
        for component, value in summary.get("energy_by_component_scaled", {}).items():
            energy_by_component[component] += float(value) * layer_count

    per_einsum = {}
    width = max(3, len(str(layer_count - 1)))
    for layer_idx in range(layer_count):
        layer_prefix = f"layer{layer_idx:0{width}d}"
        for entry in layer_entries:
            summary = entry["mapping_result"]["mapping_cost_summary"]
            for einsum_name, bucket in summary.get("per_einsum", {}).items():
                per_einsum[f"{layer_prefix}:{einsum_name}"] = _json_ready(bucket)

    combined["energy_by_component_scaled"] = dict(sorted(energy_by_component.items()))
    combined["per_einsum"] = per_einsum
    combined["decomposed_model"] = config
    return combined


def _combine_decomposed_detail(layer_entries, key, layer_count):
    return {
        "layer_multiplier": layer_count,
        "one_layer": {
            entry["spec"]["name"]: entry["mapping_result"][key]
            for entry in layer_entries
        },
    }


def map_decomposed_gpt3_einsums(
    workload_specs,
    params,
    network_proxies,
    af,
    ...
    for spec in workload_specs:
        mapping_result = map_and_extract(
            spec["path"],
            params,
            network_proxies,
            af,
        ...
        layer_entries.append({"spec": spec, "mapping_result": mapping_result})

    mapping_cost_summary = _combine_decomposed_mapping_cost_summaries(
        layer_entries, layer_count, config
    )

    yaml_parts = [
        "# Decomposed GPT-3 175B mapping",
        f"# model: {config['model']}",
        f"# layer_multiplier: {layer_count}",
    ...
    for entry in layer_entries:
        yaml_parts.extend(
            [
                f"# einsum: {entry['spec']['name']}",
                entry["mapping_result"]["mapping_yaml"],
                "",
            ]
        )
    mapping_yaml = "\n".join(yaml_parts)
    ...
    for layer_idx in range(layer_count):
        layer_prefix = f"layer{layer_idx:0{width}d}"
        for entry in layer_entries:
            result = entry["mapping_result"]
            transfers.extend(
                _copy_transfer_with_einsum_prefix(transfer, layer_prefix)
                for transfer in result["transfers"]
            )
            decisions.extend(
                _copy_decision_with_einsum_prefix(decision, layer_prefix)
                for decision in result["collective_decisions"]
            )

    return {
        "compute_energy": float(
            sum(entry["mapping_result"]["compute_energy"] for entry in layer_entries)
            * layer_count
        ),
        "compute_latency": float(
            sum(entry["mapping_result"]["compute_latency"] for entry in layer_entries)
            * layer_count
        ),
        "transfers": transfers,
        ...
        "mapping_sha256": _mapping_sha256(mapping_yaml),
        "mapping_cost_summary": mapping_cost_summary,
        "mapping_energy": _combine_decomposed_detail(
            layer_entries, "mapping_energy", layer_count
        ),
        "mapping_latency": _combine_decomposed_detail(
            layer_entries, "mapping_latency", layer_count
        ),
        "mapping_actions": _combine_decomposed_detail(
            layer_entries, "mapping_actions", layer_count
        ),
        "decomposed_model": config,
    }
...
def run_feedback_loop(
    ...
    elif _is_decomposed_gpt3_workload(params):
        mapping_dir = make_run_dir() / "mappings" / "GPT3_175B"
        mapping_dir.mkdir(parents=True, exist_ok=True)

    decomposed_workload_specs = None
    if _is_decomposed_gpt3_workload(params):
        decomposed_workload_specs = _write_gpt3_einsum_workloads(
            mapping_dir / "generated_workloads"
        )

    for iteration in range(1, MAX_PROXY_ITERS + 1):
        if decomposed_workload_specs is None:
            mapping_result = map_and_extract(
                path, params, network_proxies, af, map_workload_to_arch
            )
        else:
            mapping_result = map_decomposed_gpt3_einsums(
                decomposed_workload_specs,
                params,
                network_proxies,
                af,
                map_workload_to_arch,
...
def main():
    ...
    if args.workload:
        exact_workloads = [(d, p, prm) for d, p, prm in workloads if args.workload == d]
        workloads = exact_workloads or [
            (d, p, prm) for d, p, prm in workloads if args.workload in d
        ]
        if not workloads:
            raise SystemExit(
    ...
    print(
        ...
        f"(map={MAP_CHIPS} chips, eval={EVAL_CHIPS} chips, independent collective costs, iterative feedback, "
